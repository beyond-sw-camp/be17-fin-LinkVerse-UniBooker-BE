...
@Transactional
@Rollback(false)
class ResourceGroupActivateIntegrationTest {
    ...
    @Test
    void activateResourceGroupTest() {
        ...
        // when: 서비스 그룹 활성화 실행
        resourceGroupWebPort.activate(authDto, existingResourceGroupId);
        ...
        assertTrue(group.getIsActive(), "리소스 그룹이 활성화되어야 합니다.");
        assertNotNull(group.getUpdatedBy(), "수정자가 설정되어야 합니다.");

        System.out.println("✅ 활성화 완료: id=" + group.getId() +
